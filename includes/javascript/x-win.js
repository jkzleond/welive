...
var MyWin=0,CurrentId=0;var zIndex=1000;
function openwin(id,title){
	var o=$('win'+id);
	if(o){
		MyWin.Title('win'+id, title);
...
function DialogWin(){
	this.Create=function(id,title,wbody){
		...
		for(var i=0;i<9;i++){
			...
			}if(i!=4&&i!=2) temp.onmousedown=function(e){MyWin.Move(Winid,e?e:window.event)}
		}
