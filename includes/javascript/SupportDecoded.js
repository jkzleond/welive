...
function GetObjByE(e){
	...
	var Obj=document.all?e.srcElement:e.target;return Obj;
}
function flashTitle(){
	clearInterval(tttt);
	flashtitle_step=1;
...
function showMsgs(id,delay){
	...
	var xx=e.clientX;
	var yy=e.clientY;eTools_msg.className='tools_msg_hover';
...
function setUnderlineStyle(tstyle,mstyle){
	for(var k in guest){
		...
		var eMessage=$("message_"+guest[k]);
		if(!obj||!eMessage)continue;obj.className='tools_underline_'+tstyle;
...
function welive_output(data){
	...
	setStatus(1);
	var allguests,onlines=0,allmsgs='',aguest,gid,isonline,row,cell,gwin;
	var newdata=data;
...
function output_win(data){
	var eHistory,eNews,newdata,aline,gid,time,sender,msg,stype,content,ctype,biu,color,style;var lines=data.split('^^^');
	...
	for(var i=0;i<lines.length;i++){
		...
		if(aline[2]){
			...
			eHistory.innerHTML=eHistory.innerHTML+newdata+'<div class="clear"></div>';eHistory.scrollTop=eHistory.scrollHeight;
		}
	}
...
function getURL(url,limit){
	if(url.substr(0,5).toLowerCase()=='href=')
		return url;
	if(!limit) limit=60;
